# triton_transformer/dropout.py
# 兼容 Triton 3.4+，去掉 **meta 写法
import torch
...
def _seeded_dropout(
    ...
    p,
    seed,
    BLOCK_SIZE: tl.constexpr,   # constexpr 替换掉 **meta
):
    ...
    x0 = tl.load(x_ptr + off0, mask=mask0, other=0.0)
    x1 = tl.load(x_ptr + off1, mask=mask1, other=0.0)
    x2 = tl.load(x_ptr + off2, mask=mask2, other=0.0)
    x3 = tl.load(x_ptr + off3, mask=mask3, other=0.0)

    # Triton 内置的四倍随机数生成器
    r0, r1, r2, r3 = tl.rand4x(seed, off0)
...
class dropout_(autograd.Function):
    ...
    def backward(cls, ctx, dy):
        return seeded_dropout(dy, ctx.p, ctx.seed), None
